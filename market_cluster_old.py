...
def read_market_daily() -> pd.DataFrame:    
    df = read_matching_files(glob_string=result_path+'/market_daily/*.feather', reader=pd.read_feather)
    df = find_compleat_market_symbols(df, compleat_only=True)
    df = df.sort_index()
    return df


def find_tradable_symbols(start_date: str, end_date: str) -> pd.DataFrame:
    df = get_dates_df(tick_type='daily', symbol='market', start_date=start_date, end_date=end_date, source='local')
