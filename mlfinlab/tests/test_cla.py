...
class TestCLA(unittest.TestCase):
    ...
    def test_lambda_for_no_bounded_weights(self):
        ...
        cla = CriticalLineAlgorithm(weight_bounds=(0, 1), calculate_expected_returns="mean")
        cla.allocate(asset_prices=self.data, solution='min_volatility', asset_names=self.data.columns)
        expected_returns = cla.returns_estimator.calculate_mean_historical_returns(asset_prices=self.data)
    ...
    def test_free_bound_weights(self):
        ...
        if (expected_returns == np.ones(expected_returns.shape) * expected_returns.mean()).all():
            expected_returns[-1, 0] += 1e-5
        x, y = cla._free_bound_weight(self.data.cov(), expected_returns, free_weights=[1]*(expected_returns.shape[0]+1))
        assert not x
        assert not y
    ...
    def test_expected_returns_equals_means(self):
        ...
        _, returns, _, _ = cla._initialise(asset_prices=data, resample_by='B', expected_asset_returns=None, covariance_matrix=None)
        assert returns[-1, 0] == 1e-5
    ...
    def test_purge_excess(self):
        ...
        with self.assertRaises(IndexError):
            ...
            lower_bounds = np.ones((self.data.shape[0], 1)) * 0
            upper_bounds = np.ones((self.data.shape[0], 1)) * 1
            cla._purge_num_err(lower_bounds, upper_bounds, tol=1e-18)
    ...
    def test_flag_true_for_purge_num_err(self):
        ...
        cla.gammas = [[1]]
        cla.free_weights = [[1]]
        cla._purge_num_err([100], [1], tol=1)
