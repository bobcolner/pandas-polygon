...
class TestNestedClusteredOptimisation(unittest.TestCase):
    """
    ...
    def test_singular_covariance_matrix():
        """
        Test NCO when passing a singular covariance matrix.
        """
        ...
        cov_matrix = np.array([[0.01, 0.002, -0.001],
                               [0.002, 0.04, -0.006],
                               [-0.001, -0.006, 0.01]])
        cov_matrix[:, 2] = 2 * cov_matrix[:, 0] + 3 * cov_matrix[:, 1]

        mu_vec = np.array([1, 1, 1]).reshape(-1, 1)

        # Finding the optimal weights
        w_cvo_ = nco.allocate_cvo(cov_matrix, mu_vec=mu_vec)

        inv_cov = np.linalg.pinv(cov_matrix)
        w_cvo = np.dot(inv_cov, mu_vec)
        w_cvo /= np.dot(mu_vec.T, w_cvo)

        np.testing.assert_almost_equal(w_cvo_, w_cvo)
