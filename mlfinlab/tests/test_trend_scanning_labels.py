...
class TestTrendScanningLabels(unittest.TestCase):
    ...
    def test_trend_scanning_labels_forward(self):
        """
        Test trend scanning labels with forward-looking window
        """
        ...
        tr_scan_labels_none = trend_scanning_labels(self.eem_close, t_events=None, observation_window=20)
        tr_scan_labels_none.dropna(inplace=True)

        self.assertTrue((tr_scan_labels == tr_scan_labels_none).all().all())

    def test_trend_scanning_labels_backward(self):
        """
        Test trend scanning labels with backward-looking window
        """

        t_events = self.eem_close.index
        tr_scan_labels = trend_scanning_labels(self.eem_close, t_events, 20, False)

        self.assertEqual(tr_scan_labels.shape[0], len(t_events))  # we have label value for all t events

        # Looking backwards, before 2008/5/22 was a strong positive trend
        self.assertTrue(
            set(tr_scan_labels.loc[pd.Timestamp(2008, 4, 29):pd.Timestamp(2008, 5, 22)].bin) == set([1]))

        self.assertEqual(tr_scan_labels.bin.value_counts()[-1], 86)  # Number of -1 labels check
        self.assertEqual(tr_scan_labels.bin.value_counts()[1], 24)

        # First 20 labels are nan because we don't have data for look-backward period (20)
        self.assertEqual(tr_scan_labels.shape[0] - tr_scan_labels.dropna().shape[0], 19)
        ...
        for int_index, (ret_v, bin_v) in zip([0, 2, 10, 20, 50],
                                             [(-0.0025, 1), (-0.0201, 1), (-0.0126, 1), (0.0185, -1), (0.0147, -1)]):
            self.assertAlmostEqual(tr_scan_labels.iloc[int_index]['ret'], ret_v, delta=1e-4)
            self.assertEqual(tr_scan_labels.iloc[int_index]['bin'], bin_v)
